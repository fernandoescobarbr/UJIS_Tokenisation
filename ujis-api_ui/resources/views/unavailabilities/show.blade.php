...
<html lang="pt">
  ...
  <body>
    ...
    <p>
      <a
        class="pill"
        href="{{ route('ui.unavailabilities.certificate', [
          'system'    => $routeParams['system'],
          'location'  => $routeParams['location'],
          'startTime' => $routeParams['startTime'],   {{-- use the RAW original value --}}
        ]) }}"
      >
    ...
    @if ($error)
      {{-- Error state: show failure message from controller --}}
      <div class="error">{{ $error }}</div>

    @elseif (!$item)
      {{-- No data: defensive branch (should rarely occur if controller guarded properly) --}}
      <p class="muted">No data available.</p>

    @else
      {{-- Success: canonical on-chain fields --}}
      <div class="card">
        <div class="row">
          <div class="col">
        ...
        <div class="row">
          <div class="col">
            ...
            <span class="mono">{{ $item['verificationHash'] ?? '-' }}</span>
          </div>
        </div>
      </div>
    @endif
